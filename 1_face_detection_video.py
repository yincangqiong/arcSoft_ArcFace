...
#--------------------------------以上部分两个函数以及主函数的几条语句不变-----------------------------------------------------------
    ...
    videoWriter = cv2.VideoWriter('test.avi',fourcc,fps,(128,128))
    for i in range(2000):
    #while True:
        _,frame = vs.read()
        cv2.imwrite('1_procession/1.jpg',frame)
        filePath='1_procession/1.jpg'
        ...
        for i in range(0, len(faceInfos)):
            ...
            #cv2.imwrite('1_procession/2.jpg',np.array(img))
